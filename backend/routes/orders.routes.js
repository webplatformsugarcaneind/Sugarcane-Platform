  ...
  try {
      ...
      try {
        ...
        if (cropListing) {
          if (newQuantity <= 0) {
            ...
            // Also update quantity_available.value if it exists
    ...
    if (status === 'accepted') {
      ...
      responseData.order = {
        ...
        isPartialFulfillment: order.isPartialFulfillment || false,
        originalQuantityRequested: order.originalQuantityRequested
      };
      
      // Add listing impact information
      if (order.isPartialFulfillment) {
        message = `Order accepted with partial fulfillment`;
        responseData.partialFulfillment = {
          originalQuantity: order.originalQuantityRequested,
          fulfilledQuantity: order.orderDetails.quantityWanted,
...
router.get('/my-orders', authorize('Farmer'), async (req, res) => {
  try {
    ...
    // Find user with both sent and received orders
    const user = await User.findById(farmerId)
      .select('sentOrders receivedOrders name email')
      .populate('sentOrders.farmerId', 'name email')
      .populate('receivedOrders.buyerId', 'name email');
    ...
    // Combine and format orders for frontend display
    const allOrders = [];
    
    // Add sent orders (where current user is buyer)
    if (user.sentOrders && user.sentOrders.length > 0) {
      user.sentOrders.forEach(order => {
        allOrders.push({
          _id: order.orderId || order._id,
          type: 'sent',
          status: order.status,
          quantity_tons: order.orderDetails?.quantityWanted || 0,
          agreed_price_per_ton: order.orderDetails?.proposedPrice || 0,
          total_amount: order.orderDetails?.totalAmount || 0,
          delivery_location: order.orderDetails?.deliveryLocation || '',
          message: order.orderDetails?.message || '',
          urgency: order.orderDetails?.urgency || 'normal',
          seller_name: order.targetFarmerName || 'Unknown Farmer',
          seller_email: order.targetFarmerEmail || '',
          listing: {
            title: `Order for Listing ${order.listingId}`,
            _id: order.listingId
          },
    ...
    // Add received orders (where current user is seller)
    if (user.receivedOrders && user.receivedOrders.length > 0) {
      user.receivedOrders.forEach(order => {
        allOrders.push({
          ...
          message: order.orderDetails?.message || '',
          urgency: order.orderDetails?.urgency || 'normal',
          buyer_name: order.buyerDetails?.name || 'Unknown Buyer',
          buyer_email: order.buyerDetails?.email || '',
          buyer_phone: order.buyerDetails?.phone || '',
          listing: {
            title: `Order for Listing ${order.listingId}`,
            _id: order.listingId
          },
    ...
    res.json({
      ...
      summary: {
        sent: user.sentOrders?.length || 0,
        received: user.receivedOrders?.length || 0,
