 ...
 * @param   id - Invitation ID
 */
router.delete('/notifications/:id', clearSingleNotification);

/**
 * @route   DELETE /api/factory/notifications
 * @desc    Clear all notifications (invitations) from database
 * @access  Private (Factory only)
 * @query   status (optional) - Filter by invitation status
 */
router.delete('/notifications', clearAllNotifications);

// ===================================
// CRUSHING STATUS API ROUTES  
// ===================================

/**
 * @route   GET /api/factory/crushing-status
 * @desc    Get the current crushing status of the factory
 * @access  Private (Factory only)
 */
router.get('/crushing-status', getCrushingStatus);
