...
router.get('/marketplace', async (req, res) => {
  try {
    ...
    const validListings = listings.filter(listing => {
      ...
      if (!hasValidFarmer) {
        console.log(`  Filtering out listing "${listing.title}" - Invalid farmer_id: ${listing.farmer_id || 'null'}`);
  ...
  try {
    ...
    if (updateData.keep_existing_images) {
      try {
        ...
        console.log(' Keeping existing images:', finalImages.length);
      } catch (e) {
    ...
    if (finalImages.length > 0 || updateData.keep_existing_images) {
      ...
      console.log(' Total images after update:', finalImages.length);
    }
...
router.get('/my-listings', protect, authorize('Farmer'), async (req, res) => {
  try {
    ...
    if (hideZeroQuantity) {
      ...
      console.log(' Filtering out zero quantity listings');
    }
