...
/**
 * @route   PUT /api/contracts/:contractId/mark-delivered
 * @desc    Mark contract as delivered (sets delivery_date to current date)
 * @access  Private (HHM or Factory - must be party to the contract)
 * @params  contractId: string (required) - Contract ObjectId
 * @example PUT /api/contracts/64f123456789abcdef123456/mark-delivered
 */
router.put('/:contractId/mark-delivered', authorize('HHM', 'Factory'), async (req, res) => {
  try {
    ...
    const isParty = contract.hhm_id.toString() === req.user._id.toString() || 
                   contract.factory_id.toString() === req.user._id.toString();
    ...
    res.status(500).json({
      success: false,
      message: 'Error marking contract as delivered',
      error: error.message
    });
  }
});
...
/**
 * @route   PUT /api/contracts/:contractId/mark-paid
 * @desc    Mark contract as paid (sets payment_date to current date and payment_status to 'paid')
 * @access  Private (HHM or Factory - must be party to the contract)
 * @params  contractId: string (required) - Contract ObjectId
 * @example PUT /api/contracts/64f123456789abcdef123456/mark-paid
 */
router.put('/:contractId/mark-paid', authorize('HHM', 'Factory'), async (req, res) => {
  try {
    const { contractId } = req.params;
    ...
    res.status(500).json({
      success: false,
      message: 'Error marking contract as paid',
      error: error.message
    });
  }
});
...
/**
 * @route   PUT /api/contracts/:contractId/mark-completed
 * @desc    Mark contract as completed (sets status to 'completed')
 * @access  Private (HHM or Factory - must be party to the contract)
 * @params  contractId: string (required) - Contract ObjectId
 * @example PUT /api/contracts/64f123456789abcdef123456/mark-completed
 */
router.put('/:contractId/mark-completed', authorize('HHM', 'Factory'), async (req, res) => {
  try {
    const { contractId } = req.params;
    ...
    contract.finalized_at = new Date();
    await contract.save();
