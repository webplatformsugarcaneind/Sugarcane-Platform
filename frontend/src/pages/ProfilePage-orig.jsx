...
const ProfilePage = () => {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      // Fix: Ensure array fields are strings for form input compatibility
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Convert comma-separated strings back to arrays before submission
      const dataToSubmit = { ...profileData };
      const arrayFields = ['skills', 'equipment', 'farmingMethods', 'certifications', 'managementOperations', 'servicesOffered', 'specialization'];
      arrayFields.forEach(field => {
        if (typeof dataToSubmit[field] === 'string') {
          dataToSubmit[field] = dataToSubmit[field].split(',').map(s => s.trim()).filter(Boolean);
        }
      });

      const response = await axios.put(apiEndpoint, dataToSubmit, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.data.profile) {
        ...
        const arrayFields = ['skills', 'equipment', 'farmingMethods', 'certifications', 'managementOperations', 'servicesOffered', 'specialization'];
        arrayFields.forEach(field => {
          if (updatedProfile[field] && Array.isArray(updatedProfile[field])) {
            updatedProfile[field] = updatedProfile[field].join(', ');
          }
        });
        ...
        <div className="pp-sticky-inner">
          ...
          <div className="pp-sticky-btns">
            ...
            <button
              ...
              disabled={saving}
              onClick={(e) => { e.target.closest('form') ? e.target.closest('form').requestSubmit() : document.querySelector('.profile-form')?.requestSubmit(); }}
            >
              {saving ? 'Saving...' : 'Save Profile'}
            </button>
          </div>
