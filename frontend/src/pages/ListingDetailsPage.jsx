...
const ListingDetailsPage = () => {
  ...
  const navigateToMarketplace = () => {
    try {
      ...
      // If navigation doesn't work, try window.location after a brief delay
      setTimeout(() => {
  ...
  const handleShowBuyForm = () => {
    ...
    const pricePerTon = listing.price_details?.base_price_per_ton || 
                        listing.price_details?.price_per_unit || 
                        listing.expected_price_per_ton || 0;
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.order && response.data.order.isPartialFulfillment) {
          ...
          message += `Original request: ${response.data.order.originalQuantityRequested} gunthas\n`;
          message += `Fulfilled amount: ${response.data.order.orderDetails.quantityWanted} gunthas\n`;
        ...
        // IMPORTANT: Refresh the listing data to check if quantity was updated or listing was removed
        try {
          const updatedListingResponse = await axios.get(`http://localhost:5000/api/listings/${listingId}`);
          
          if (updatedListingResponse.data.success) {
            // Update the listing state with new data
            setListing(updatedListingResponse.data.data);
          } else {
            // Listing might have been removed due to quantity depletion
            console.log('Listing may have been removed due to quantity depletion');
            alert('📋 This listing has been removed as the quantity is now fully allocated. Redirecting to marketplace...');
            setTimeout(() => {
              navigateToMarketplace();
            }, 2000);
          }
        } catch (refreshError) {
          // If 404, listing was removed
          if (refreshError.response?.status === 404) {
            ...
            console.log('Error refreshing listing data:', refreshError);
  ...
  const handleDeleteListing = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        alert('Please login to delete your listing');
        navigate('/login');
        return;
      }
      ...
      if (response.data.success) {
        ...
        alert('❌ Failed to delete listing: ' + response.data.message);
      }
    } catch (err) {
            ...
            <div className="farm-images-section">
              ...
              <div className="farm-images-grid">
                {listing.farm_images.map((image, index) => {
                  ...
                  const imageAlt = typeof image === 'object' && image.caption 
                    ? image.caption 
                    : `Farm ${index + 1}`;
                  
                  return (
                    <div key={index} className="farm-image-container">
                      <img 
                        src={imageUrl} 
                        alt={imageAlt}
                        ...
                        <div className="buyer-info">
                          ...
                          <div className="buyer-details">
                            ...
                            {order.buyer?.location && (
                              <p className="buyer-location">ï¿½ {order.buyer.location}</p>
                            )}
                          </div>
                        </div>
