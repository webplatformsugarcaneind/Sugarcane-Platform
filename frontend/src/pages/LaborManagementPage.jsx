...
const LaborManagementPage = () => {
  ...
  const handleViewProfile = (worker) => {
    ...
    // Navigate to labour profile view page with worker data

    navigate(`/hhm/labour/${worker._id}`, {
  ...
  const handleOpenManageModal = (labour) => {
    ...
    console.log('🔧 Current showManageModal state:', showManageModal);
  ...
  // Handle closing manage assignment modal
  ...
  // Handle releasing worker (ending contract)
  ...
  const handleReleaseWorker = async () => {
    ...
    const confirmRelease = window.confirm(
      ...
      `Are you sure you want to end the contract with ${selectedLabour.name}?\n\n` +

      `This will:\n` +

      `• Make them available to other HHMs\n` +

      `• Remove them from your hired workers list\n` +

      `• End their current assignment`
    ...
    try {
      ...
      console.log('🔓 Releasing worker:', selectedLabour.name);

      

      const response = await axios.post('/api/hhm/release-worker', {

        workerId: selectedLabour._id
      ...
      console.log('✅ Worker released:', response.data);

      

      alert(`✅ Contract ended successfully!\n${selectedLabour.name} is now available for other HHMs to hire.`);
      ...
      // Refresh the my labours list

      await fetchMyLabours();

      

    } catch (err) {

      console.error('❌ Error releasing worker:', err.response?.data || err.message);

      const errorMessage = err.response?.data?.message || 'Failed to end contract. Please try again.';

      alert(`❌ Error: ${errorMessage}`);

    } finally {

      setReleasingWorker(false);
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    try {
      ...
      if (!scheduleForm.requiredSkills || scheduleForm.requiredSkills.trim() === '') {
        ...
        setFieldErrors({ requiredSkills: true });
      ...
      // Check if skills array will have at least one valid entry

      const skillsArray = scheduleForm.requiredSkills
      ...
      if (skillsArray.length === 0) {
        ...
        setErrorMessage('❌ Required Skills cannot be empty. Please enter at least one valid skill.');
  ...
  return (
    ...
    <div style={styles.container}>
      ...
      {showManageModal && (
        ...
        <div style={styles.modalOverlay} onClick={handleCloseManageModal}>
          ...
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              ...
              {selectedLabour && (
                ...
                <div style={styles.manageWorkerInfo}>
                  ...
                  <div style={styles.workerDetails}>
                    ...
                    <h3 style={styles.modalWorkerName}>{selectedLabour.name}</h3>

                    <p style={styles.modalWorkerEmail}>{selectedLabour.email}</p>

                    {selectedLabour.phone && (

                      <p style={styles.modalWorkerPhone}>📞 {selectedLabour.phone}</p>
                    ...
                    <div style={styles.contractInfo}>
                      ...
                      <h4 style={styles.contractTitle}>📋 Current Assignment</h4>

                      <div style={styles.contractDetails}>

                        <p><strong>Job:</strong> {selectedLabour.schedule?.title || 'N/A'}</p>

                        <p><strong>Location:</strong> {selectedLabour.schedule?.location || 'N/A'}</p>

                        <p><strong>Wage:</strong> ₹{selectedLabour.schedule?.wageOffered || 'N/A'}/day</p>

                        {selectedLabour.schedule?.startDate && (

                          <p><strong>Start Date:</strong> {new Date(selectedLabour.schedule.startDate).toLocaleDateString()}</p>

                        )}

                        {selectedLabour.reviewedAt && (

                          <p><strong>Hired On:</strong> {new Date(selectedLabour.reviewedAt).toLocaleDateString()}</p>
                    ...
                    {selectedLabour.skills && selectedLabour.skills.length > 0 && (
              ...
              <div style={styles.manageActions}>
                ...
                <div style={styles.actionWarning}>
                  ...
                  <h4 style={styles.warningTitle}>⚠️ End Contract</h4>

                  <p style={styles.warningText}>

                    Ending the contract will:

                  </p>

                  <ul style={styles.warningList}>

                    <li>Remove {selectedLabour?.name} from your hired workers list</li>

                    <li>Make them available for other HHMs to hire</li>

                    <li>End their current assignment immediately</li>
            ...
            <div style={styles.modalFooter}>
              ...
              <button
                ...
                style={styles.modalReleaseButton}
